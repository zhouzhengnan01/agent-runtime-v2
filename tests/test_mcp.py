...
def test_test_users_mcp_can_keep_returning_rows_for_loop_smoke(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_mcp_runtime(tmp_path, monkeypatch)
    client = TestClient(create_app())
    session_id = "test-infinite-loop-session"
    ...
    for index in range(5):
        ...
        result = called.json()["result"]
        assert result["isError"] is False
        assert result["structuredContent"]["mode"] == "always_present"
        counts.append(result["structuredContent"]["count"])
