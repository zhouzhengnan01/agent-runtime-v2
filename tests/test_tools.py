...
def test_tool_invocation_service_keeps_manual_tools_and_skills_as_providers(
    tmp_path: Path,
) -> None:
...
""",
    ...
    registry = ToolRegistry(
        tmp_path, artifact_store=store, skill_runner=SkillRunner(store)
    )
    service = ToolInvocationService(
        registry=registry, artifact_store=store, skill_runner=SkillRunner(store)
    )
    ...
    skill = service.call_tool(
        "markdown-rendering",
        {
            "_thread_id": "tool-skill-test",
            "title": "Tool Layer",
            "summary": "Skill provider output",
        },
    )
...
def test_mcp_streamable_http_provider_calls_remote_tool(
...
def test_mcp_streamable_http_provider_lists_tools_from_sse(
    monkeypatch: MonkeyPatch,
) -> None:
    original_client = httpx.Client
...
def test_mcp_streamable_http_provider_builds_runtime_tools_from_server(
    monkeypatch: MonkeyPatch,
) -> None:
    original_client = httpx.Client
...
def test_mcp_streamable_http_provider_builds_runtime_resource_tools_from_server(
    monkeypatch: MonkeyPatch,
) -> None:
    original_client = httpx.Client
            ...
            return httpx.Response(
                ...
                json={
                    ...
                    "result": {
                        "resourceTemplates": [
                            {
                                "uriTemplate": "jetlinks://ces/QueryList{?terms,sorts}",
                                "name": "查询用户信息（不分页）",
                                "description": "指定查询条件，分页参数，排序规则等",
                            }
                        ]
                    },
                },
    ...
    assert [tool.name for tool in tools] == [
        "jetlinks-session__mcp_list_resources",
        "jetlinks-session__mcp_list_resource_templates",
        "jetlinks-session__mcp_read_resource",
    ]
    assert tools[0].source["operation"] == "list_resources"
    assert tools[1].source["operation"] == "list_resource_templates"
    assert tools[2].source["operation"] == "read_resource"
    assert tools[2].input_schema["properties"]["uri"]["enum"] == [
        "file:///project/README.md"
    ]
    assert "README <file:///project/README.md>" in tools[2].description
    assert "jetlinks://ces/QueryList{?terms,sorts}" in tools[2].description
    assert seen_methods == [
        "initialize",
        "notifications/initialized",
        "resources/list",
...
def test_mcp_streamable_http_provider_reads_remote_resource(
    monkeypatch: MonkeyPatch,
) -> None:
    original_client = httpx.Client
    seen_methods: list[str] = []
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if payload["method"] == "resources/read":
            assert payload["params"] == {"uri": "file:///project/README.md"}
            return httpx.Response(
                200,
                json={
                    "jsonrpc": "2.0",
    ...
    tool = ToolDefinition(
        name="jetlinks-session__mcp_read_resource",
        title="Read resource",
    ...
    result = McpStreamableHttpToolProvider().call(
        tool, {"uri": "file:///project/README.md"}
    )

    assert result.is_error is False
    assert "Resource file:///project/README.md" in result.content[0]["text"]
    assert "# Runtime" in result.content[0]["text"]
    assert (
        result.structured_content["contents"][0]["uri"] == "file:///project/README.md"
    )
    assert seen_methods == ["initialize", "notifications/initialized", "resources/read"]


def test_tool_invocation_service_caches_runtime_mcp_tool_discovery(
    monkeypatch: MonkeyPatch,
) -> None:
    original_client = httpx.Client
    seen_methods: list[str] = []
    ...
    service = ToolInvocationService()
    servers = [
        {"name": "jetlinks-session", "url": "https://example.test/mcp", "type": "http"}
    ]
...
def test_tool_invocation_service_reports_runtime_mcp_policy_failures(
    monkeypatch: MonkeyPatch,
) -> None:
...
def test_tool_invocation_service_supports_runtime_mcp_allowed_host_policy(
    monkeypatch: MonkeyPatch,
) -> None:
...
""",
    ...
    service = ToolInvocationService(root_dir=tmp_path)
    servers = [
        {
            "name": "stdio-session",
            "type": "stdio",
...
def test_tool_invocation_service_discovers_and_reads_runtime_stdio_mcp_resources(
    tmp_path: Path,
    monkeypatch: MonkeyPatch,
) -> None:
    server_script = tmp_path / "stdio_mcp_resources.py"
    server_script.write_text(
        """
from __future__ import annotations
...
""",
    ...
    assert [tool.name for tool in discovery.tools] == [
        "stdio-session__mcp_list_resources",
        "stdio-session__mcp_list_resource_templates",
        "stdio-session__mcp_read_resource",
    ]
    result = service.call_tool(
        "stdio-session__mcp_read_resource",
        {
    ...
    assert result.is_error is False
    assert "Resource file:///runtime/config.json" in result.content[0]["text"]
    assert '"enabled": true' in result.content[0]["text"]


def test_tool_invocation_service_requires_runtime_stdio_mcp_opt_in(
    monkeypatch: MonkeyPatch,
) -> None:
...
def test_mcp_streamable_http_provider_does_not_send_unrelated_llm_key(
    monkeypatch: MonkeyPatch,
) -> None:
