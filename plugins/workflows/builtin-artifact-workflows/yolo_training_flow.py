...
DEFAULT_SELECTED_SKILLS = ["image-composite-generation", "data-auto-annotation", "gpu-training-orchestrator"]


def _runtime_options(value: Any | None = None) -> Any:
...
def _agent_run_result(**kwargs: Any) -> Any:
    from app.schemas import AgentRunResult

    return AgentRunResult(**kwargs)


def _verification_result(**kwargs: Any) -> Any:
...
def _event_recorder(**kwargs: Any) -> Any:
    from app.core.events import EventRecorder

    return EventRecorder(**kwargs)
...
def _openai_compatible_client(agent_config: Any, runtime_options: Any) -> Any:
    from app.core.llm.openai_compatible import OpenAICompatibleClient

    return OpenAICompatibleClient(agent_config, runtime_options=runtime_options)
...
class YoloTrainingWorkflow:
    ...
    def run_with_events(
        ...
        runtime_options = _runtime_options(runtime_options)
        workflow = workflow_name or WORKFLOW_NAME
        paths = self.artifact_store.prepare_thread(thread_id)
        recorder = _event_recorder(agent=agent_config.name, thread_id=paths.thread_id, on_emit=on_event)
        ...
        if data_prep_returncode != 0:
            ...
            result = _agent_run_result(
                ...
                reply=reply,
                artifacts=outputs,
                verification=_verification_result(passed=False, retry_count=0, checks=[], failed_checks=["data-auto-annotation failed"]),
                metadata={
                    "workflow": workflow,
        ...
        if not training_enabled:
            ...
            result = _agent_run_result(
                ...
                reply=reply,
                artifacts=outputs,
                verification=_verification_result(passed=True, retry_count=0, checks=[], failed_checks=[]),
                metadata={
                    "workflow": workflow,
...
def _generate_model_controlled_reply(
    *,
    ...
    llm = _openai_compatible_client(agent_config, runtime_options)
    recorder.emit(
        "llm.started",
        {
            "model": llm.model,
    ...
    *,
    ...
    if llm.configured:
        try:
            ...
            reply = llm.complete_sync(system_prompt, [_message(role="user", content=user_text)]).strip()
            if reply:
...
def _extract_yolo_training_request_spec(
    ...
    *,
    ...
    try:
        raw = llm.complete_sync(system_prompt, [_message(role="user", content=user_text)])
        payload = _parse_json_object(raw)
...
def _fallback_workflow_capabilities(skill_name: str) -> set[str]:
    fallback = {
        ...
        "image-composite-generation": {"image_generation"},
...
def _extract_annotation_labels(user_text: str) -> list[str]:
    ...
    if not raw:
        return []
    raw = re.split(
        r"\s+(?:conda_env_name|training_task|model|epochs|imgsz|batch|device|workers|patience|dataset\.split\.(?:train|val|test))\s*[:=]",
        raw,
        maxsplit=1,
        flags=re.IGNORECASE,
    )[0]
...
def _resolve_coco_output(paths: ThreadPaths, outputs: list[Any]) -> Path:
    for artifact in outputs:
        ...
        if not isinstance(artifact_path, str):
            continue
        try:
            local = _artifact_store().resolve_virtual_path(paths.thread_id, artifact_path)
        except Exception:
            continue
