...
def _collect_image_paths(value: Any) -> list[Path]:
    ...
    if isinstance(value, str):
        ...
        if p.is_dir():
            ...
            paths.append(p)
        elif p.is_file() and _is_archive_path(p):
            extract_dir = _extract_archive(p)
            paths.extend(_collect_image_paths(str(extract_dir)))
    ...
    if path.suffix.lower() == ".zip":
        with zipfile.ZipFile(path) as archive:
            for member in archive.infolist():
                if member.is_dir():
                    continue
                destination = _safe_extract_target(target, member.filename)
        ...
        with tarfile.open(path) as archive:
            for member in archive.getmembers():
                if not member.isfile():
                    continue
                destination = _safe_extract_target(target, member.name)
                destination.parent.mkdir(parents=True, exist_ok=True)
...
def resolve_image_paths(args: argparse.Namespace) -> list[Path]:
    ...
    if args.input_json:
        ...
        parsed = _prepare_composite_image_input(parsed)
...
def _prepare_composite_image_input(parsed: dict[str, Any]) -> dict[str, Any]:
    if parsed.get("merge_with_generated") is False:
        return parsed
    workspace_dir = _path_from_payload(parsed, "workspace_dir")
    outputs_dir = _path_from_payload(parsed, "outputs_dir")
    uploads_dir = _path_from_payload(parsed, "uploads_dir")
    if workspace_dir is None and outputs_dir is None and uploads_dir is None:
        return parsed

    explicit_paths: list[Path] = []
    for key in ("image_path", "image_paths", "path", "paths", "input_dir", "input_path"):
        if key in parsed:
            explicit_paths.extend(_resolve_virtual_collect(parsed[key], uploads_dir, outputs_dir, workspace_dir))
    if not explicit_paths and uploads_dir is not None:
        explicit_paths.extend(_collect_image_paths(str(uploads_dir)))
        for archive in _archive_files(uploads_dir):
            explicit_paths.extend(_collect_image_paths(str(archive)))

    generated_paths: list[Path] = []
    for root in (workspace_dir, outputs_dir):
        if root is None:
            continue
        for candidate in (root / "generated_images", root):
            if candidate.exists():
                generated_paths.extend(
                    path
                    for path in _collect_image_paths(str(candidate))
                    if "generated" in path.name.lower() or "generated_images" in str(path.parent).lower()
                )

    all_paths = _dedupe_paths([*explicit_paths, *generated_paths])
    if not all_paths:
        return parsed
    merge_root = (workspace_dir or outputs_dir or uploads_dir) / "merged_dataset"
    images_dir = merge_root / "images"
    ...
    for source in all_paths:
        ...
        if target.exists():
            ...
            target = images_dir / f"{stem}_{copied + 1}{suffix}"
        shutil.copy2(source, target)
        copied += 1
        if "generated" in source.name.lower() or "generated_images" in str(source.parent).lower():
            generated_count += 1
    summary = {
        "uploaded_or_existing_images": copied - generated_count,
        "generated_images": generated_count,
        "merged_images": copied,
        "merged_images_dir": str(images_dir.resolve()),
    }
    merge_root.mkdir(parents=True, exist_ok=True)
    (merge_root / "merge_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    merged = dict(parsed)
    merged["image_path"] = str(images_dir.resolve())
    merged["merged_dataset_root"] = str(merge_root.resolve())
    merged["merge_summary"] = summary
    return merged


def _path_from_payload(parsed: dict[str, Any], key: str) -> Path | None:
...
def _resolve_virtual_collect(value: Any, uploads_dir: Path | None, outputs_dir: Path | None, workspace_dir: Path | None) -> list[Path]:
    if isinstance(value, list):
        paths: list[Path] = []
        for item in value:
            paths.extend(_resolve_virtual_collect(item, uploads_dir, outputs_dir, workspace_dir))
        return paths
    if not isinstance(value, str):
        return []
    resolved = _resolve_virtual_path(value, uploads_dir, outputs_dir, workspace_dir)
    return _collect_image_paths(str(resolved))


def _resolve_virtual_path(value: str, uploads_dir: Path | None, outputs_dir: Path | None, workspace_dir: Path | None) -> Path:
    raw = value.strip()
    direct = Path(raw)
    if direct.exists():
        return direct
    ...
    for prefix, root in (
        ...
        ("d:/mnt/user-data/uploads", uploads_dir),
        ("/mnt/user-data/outputs", outputs_dir),
        ("d:/mnt/user-data/outputs", outputs_dir),
        ("/mnt/user-data/workspace", workspace_dir),
        ("d:/mnt/user-data/workspace", workspace_dir),
    ):
        if root is None:
            continue
        if lowered == prefix:
        ...
        if lowered.startswith(prefix + "/"):
            return root / normalized[len(prefix):].lstrip("/")
    return direct
...
def _archive_files(root: Path) -> list[Path]:
    if not root.exists():
        return []
    return [path for path in root.rglob("*") if path.is_file() and _is_archive_path(path)]
