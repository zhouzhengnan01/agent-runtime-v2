...
def score_skill(
    ...
    if _first_image_path(attachments):
        score += 30
    elif _first_archive_path(attachments):
        score += 25
    return score
...
def build_spec(
    ...
    image_path = _first_image_path(attachments) or _first_archive_path(attachments)
...
def _first_archive_path(attachments: list[Any]) -> str:
    for attachment in attachments:
        payload = _attachment_payload(attachment)
        name = str(payload.get("name") or payload.get("filename") or "").lower()
        path = str(payload.get("path") or payload.get("local_path") or payload.get("file_path") or "").strip()
        lowered_path = path.lower()
        if path and any(name.endswith(ext) or lowered_path.endswith(ext) for ext in ARCHIVE_EXTENSIONS):
            return path
    return ""
