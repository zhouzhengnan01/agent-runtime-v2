...
def _normalize_runtime_payload(payload: dict) -> dict:
    ...
    task = {
        "image1": task_spec.get("image1") or spec.get("image1") or spec.get("input_image1") or spec.get("background_image") or (images[0] if len(images) > 0 else ""),
        "image2": task_spec.get("image2") or spec.get("image2") or spec.get("input_image2") or spec.get("foreground_image") or (images[1] if len(images) > 1 else ""),
        "prompt": task_spec.get("prompt") or spec.get("prompt") or spec.get("composite_prompt") or spec.get("generation_prompt") or _prompt_from_text(str(spec.get("overrides_text") or "")) or "",
...
def main() -> None:
    ...
    cmd = [
        ...
        "--url", str(model.get("api_url", "") or "https://www.tokencloud.yun/v1/images/generations"),
        "--token", str(model.get("token", "") or model.get("api_key", "") or "sk-6qbqlTAZV7qVszLplZhxZH2yvQwSMRotrOT8wr9aVPII8BYo"),
        "--model", str(model.get("model", "") or "wan2.7-image-pro"),
        "--image1", image1,
        "--image2", image2,
        "--prompt", prompt,
        "--output-dir", str(output_path),
        "--size", str(model.get("size", "") or "768*768"),
    ...
    print("[image-dataset-composite] Starting image composition task...")
    start = time.time()
