...
def score_skill(
    ...
    if skill_name != "image-dataset-generation" or skill_name not in allowed_skills:
        return 0
    ...
    if len(_image_or_folder_paths(attachments)) >= 2:
        score += 35
    return score
...
def build_spec(
    ...
    image_paths = _image_or_folder_paths(attachments)
...
def _image_or_folder_paths(attachments: list[Any]) -> list[str]:
    paths: list[str] = []
