...
This reference defines the standard blueprint for generating one visualization big-screen page progressively.

The generated result is not a frontend code project.

The initialization response must include:
...
Each region component response must include:
...
Both background SVG content and region component placement must be generated from the same fixed layout regions.

The background SVG content must be uploaded through `fileService / UploadFile`, and the returned file id must be written to `pageJson.canvas.backgroundImage.fileId`.
...
## 2. Progressive Outputs

This blueprint supports two normal response modes.

Initialization response responsibilities:

```text
uploaded background SVG content = decorative and structural SVG background
pageJson                        = platform-compatible canvas JSON with empty components
blueprint                       = simplified fixed layout blueprint for later turns
```

Region component response responsibilities:

```text
regionId                        = requested blueprint region id
components                      = List<ComponentInfo> using real platform component JSON
resources/*.resource.json        = ECharts resource entity JSON for custom-chart components when generated
```

The SVG background content must be generated from the fixed regions in this document, uploaded through the file command, and referenced by `pageJson.canvas.backgroundImage.fileId`.

Returned region components must be placed inside the corresponding region `contentBox` or slot.

Every `custom-chart` page component in a region response must have a matching resource entity JSON object.

---

## 3. Platform Page JSON Structure

The generated initialization `pageJson` must use this exact root structure:
...
- `pageJson.canvas.backgroundImage.fileId` must be the `id` returned by `fileService / UploadFile`.
- Do not leave `pageJson.canvas.backgroundImage.fileId` empty after successful background upload.
- Do not add non-platform fields into `canvas`.
- Do not write layout debug fields into `pageJson`.
...
The skill must generate SVG content, Base64 encode the SVG UTF-8 bytes, call the upload command, and write the returned `id` into `pageJson.canvas.backgroundImage.fileId`.
...
generate random resource id
  ...
  -> add component to the current region response components array
```
...
- Do not write the ECharts script directly into returned component JSON.
...
- The returned region component JSON must already contain the generated resource id.
...
The resource entity JSON must not be embedded into returned component JSON.
...
- Do not call this command for `text`, `table`, `dateTime`, `tabs`, background SVG content, or initialization `pageJson`.
...
- If the command call fails, keep the generated files and report the command failure.

Background SVG upload is only part of initialization. Before returning an initialization response, upload the generated background SVG content:
...
Write the returned upload result `id` to `pageJson.canvas.backgroundImage.fileId`.

After `pageJson` has been generated, background SVG upload succeeds, and `pageJson.canvas.backgroundImage.fileId` is set, return `{ "pageJson": pageJson, "blueprint": simplifiedBlueprint }`.

After one region's components have been generated and every custom-chart resource command for that region has been attempted, return `{ "regionId": requestedRegionId, "components": componentList }`.
...
10. Add the component to the current region response `components` array.
...
dark blue gradient panel fill
panel outer border
border glow or border flow highlight
title safe area decoration
title underline gradient
lightweight top-right decoration outside the title text area
panel corner accents
optional bottom glow
optional low-opacity contentBox boundary only
```

Panel visual style:

```text
Use a cyber panel style similar to this structure:
- panel body: dark blue gradient, about 0.82-0.9 opacity.
- outer border: cyan/blue stroke with low-opacity base layer and brighter highlight layer.
- title area: reserve the top band from region.y to contentBox.y; draw a short gradient underline inside the title band.
- top-right decoration: short thin paths and small cyan dots only, placed away from the title text safe area.
- content area: only a low-opacity boundary rectangle is allowed; do not alter the content area's bottom fill.
- corners: four short cyan corner strokes with low opacity.
- bottom: a short subtle cyan glow line is allowed on the panel border.
```

Title rules:

```text
Do not write literal panel title text in backgroundSvg.
The actual panel title text must be generated later as a platform text component in the region stage.
The background SVG may reserve title text safe space and may draw the title underline/decorations.
Do not let decoration paths cross the title text safe area.
```

ContentBox cleanliness rules:

```text
The area inside every region contentBox must remain clean for platform components.
Do not draw grid lines, background line textures, route lines, decorative section dividers, business placeholders, radial glows, chart-like marks, or map guide marks inside any contentBox.
Do not draw background lines or center visual base decoration inside center_main_panel.contentBox.
If a contentBox boundary is needed, use only a low-opacity stroke rectangle with fill="none".
```
...
literal panel title text
grid lines inside contentBox
decorative divider lines inside contentBox
radial glow inside contentBox
center visual base decoration inside center_main_panel.contentBox
```
...
4. Write FileInfo.id to pageJson.canvas.backgroundImage.fileId.
```
...
An initialization response is valid only when all conditions are true:

```text
response root has pageJson
response root has blueprint
pageJson root has canvas
pageJson root has components
pageJson must not be wrapped in page
pageJson.components is []
pageJson.canvas.width === 1920
pageJson.canvas.height === 1080
pageJson.canvas.scale === 0.64
pageJson.canvas.sizeKey === "pc"
pageJson.canvas.adaptationType === "AUTO"
...
pageJson.canvas.backgroundImage.fileId equals background upload result id
background SVG content has viewBox 0 0 1920 1080
background SVG content has one data-region-id group for every region
blueprint.id is standard-1920x1080-v1
blueprint regions include id, name, role, x, y, width, height, contentBox, and slots
preview.html does not exist
screen-blueprint.json does not exist
visualizationService:project / CreateBigScreenProject has not been called
```

A region component response is valid only when all conditions are true:

```text
response root has regionId
response root has components
regionId equals the requested blueprint region id
components is a List<ComponentInfo>
every component is cloned from a real platform component template
every component id is unique against ids provided in the current orchestration state
every component writes position and size through componentProps.style
every component is placed inside the requested region contentBox
every slotted component is placed inside its slot
only the five allowed platform component categories are used
component JSON does not contain regionId, slotId, position, size, or debug fields unless the real template already contains them
every custom-chart component has a matching resource entity JSON object
...
If validation fails, fix all related artifacts consistently:

```text
current response JSON
generated-bigscreen/resources/*.resource.json when custom-chart is involved
```

Do not only fix one side when coordinates or resource bindings are inconsistent.

---

## 18. Final Response Rules

For initialization, return only:
...
For one-region generation, return only:
...
background SVG content -> fileService / UploadFile -> pageJson.canvas.backgroundImage.fileId
fixed blueprint regions -> region response components
```
...
generated region components -> return {regionId, components}
```
