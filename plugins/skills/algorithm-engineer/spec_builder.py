...
DEFAULT_SKILL = "algorithm-engineer-app"
...
def score_skill(
    ...
    for keyword in SKILL_KEYWORDS[skill_name]:
        if keyword in text:
            score += 24
    if skill_name == "algorithm-engineer-app" and any(word in text for word in ("完整", "应用", "整合", "工作台", "全流程搞定")):
        score += 90
    if skill_name == "algorithm-engineer" and any(word in text for word in ("全流程", "端到端", "搞定", "算法工程师")):
        score += 60
    if attachments:
        score += 6
    return score
