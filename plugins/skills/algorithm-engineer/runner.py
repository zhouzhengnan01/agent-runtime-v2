...
SKILL_TITLES = {
    "algorithm-engineer-app": "完整算法工程师应用工作台",
    "algorithm-engineer": "算法工程师全流程方案",
...
def _payload(skill_name: str, spec: dict[str, Any]) -> dict[str, Any]:
    ...
    if skill_name == "algorithm-engineer-app":
        return _algorithm_engineer_app_payload(base, spec)
...
def _summary(skill_name: str, base: dict[str, Any]) -> str:
    if skill_name == "algorithm-engineer-app":
        return "把算法工程师日常工作收敛成一个应用入口：任务澄清、数据治理、算法选型、GPU 训练、评估上线和实验台账。"
    if skill_name == "algorithm-engineer":
        return "覆盖现状盘点、算法调研、数据治理、benchmark、正式训练、评估、上线评审和实验台账。"
...
def _markdown(skill_name: str, title: str, spec: dict[str, Any], payload: dict[str, Any]) -> str:
    if skill_name == "algorithm-engineer-app":
        return _algorithm_engineer_app_markdown(title, spec, payload)
    lines = [
        f"# {title}",
        "",
        f"- Skill: `{skill_name}`",
...
def _algorithm_engineer_app_payload(base: dict[str, Any], spec: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        **base,
        "summary": _summary("algorithm-engineer-app", base),
        "dashboard": dashboard,
        "stages": stages,
        ...
        "deliverables": ["algorithm-engineer-app.md", "algorithm-engineer-app.json", "experiment_ledger.yaml", "deployment_review.md"],
...
def _algorithm_engineer_app_markdown(title: str, spec: dict[str, Any], payload: dict[str, Any]) -> str:
