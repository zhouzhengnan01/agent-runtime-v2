...
def _normalize_runtime_payload(payload: dict) -> dict:
    ...
    task = {
        "input_image": task_spec.get("input_image") or spec.get("input_image") or spec.get("image_path") or _first_attachment_path(spec),
        "prompt": task_spec.get("prompt") or spec.get("prompt") or spec.get("generation_prompt") or _prompt_from_text(str(spec.get("overrides_text") or "")) or spec.get("task") or "",
        "output_dir": task_spec.get("output_dir") or spec.get("output_dir") or ctx.get("output_dir") or outputs_dir or str(Path(__file__).resolve().parents[2] / "outputs"),
    }
