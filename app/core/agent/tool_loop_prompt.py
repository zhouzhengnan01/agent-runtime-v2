...
COMPOSITE_SKILL_GUIDANCE = (
    ...
    "are satisfied. If a stage's required inputs are missing, ask the user for "
    "only those missing inputs before calling tools. When the user has uploaded "
    "files but has not supplied semantic parameters such as a generation prompt, "
    "class labels, or training hyperparameters, pause and ask for them instead "
    "of guessing defaults. After each child skill call, inspect the returned "
    "artifacts and use their paths as inputs to later stages."
)
...
def render_composite_skill_context(item: dict[str, Any]) -> str:
    ...
    if isinstance(stages, list) and stages:
        ...
        for stage in stages:
            ...
            requires = string_list(stage.get("requires"))
            produces = string_list(stage.get("produces"))
            require_text = f"; requires {', '.join(requires)}" if requires else ""
            produce_text = f" -> produces {', '.join(produces)}" if produces else ""
            label = stage_id or skill
            lines.append(f"- {label}: use {skill}{require_text}{produce_text}".strip())
    if done_when:
        lines.append("Done when:")
        lines.extend(f"- {condition}" for condition in done_when)
    handoff = string_list(item.get("handoff_rules"))
    if handoff:
        lines.append("Handoff rules:")
        lines.extend(f"- {rule}" for rule in handoff)
    return "\n".join(lines)
