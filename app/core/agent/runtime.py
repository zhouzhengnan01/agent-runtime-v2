...
class AgentRuntime:
    ...
    async def _stream_workflow_events(
        ...
        *,
        workflow_lookup_name: str | None = None,
        public_workflow_name: str | None = None,
        ...
        def run_workflow() -> None:
            ...
            lookup_name = workflow_lookup_name or execution.workflow_name or ""
            workflow = self.workflow_registry.get(lookup_name)
            if workflow is None:
                raise ValueError(f"Workflow is not registered: {lookup_name}")
            try:
                final_result, returned_events = workflow.run_with_events(
                    ...
                    thread_id=execution.paths.thread_id,
                    on_event=on_event,
                    workflow_name=public_workflow_name or execution.workflow_name or "agent_loop",
                    runtime_options=execution.request.runtime_options,
                )
    ...
    def _app_template_runtime_option_updates(self, runtime_options: RuntimeOptions) -> dict[str, object]:
        ...
        for option_name in (
            ...
            if option_name in explicit or getattr(runtime_options, option_name) is not None:
                continue
            value = template_options.get(option_name)
            if value is not None:
                updates[option_name] = value
        if "mode" not in explicit and runtime_options.mode is None:
            value = template_options.get("mode")
            if value is not None:
                updates["mode"] = value
        if "config_options" not in explicit and not runtime_options.config_options:
            value = template_options.get("config_options")
            if isinstance(value, dict):
                updates["config_options"] = dict(value)
