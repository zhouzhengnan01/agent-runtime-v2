...
def required_inputs_for_request(request: ChatRequest) -> list[dict[str, Any]]:
    ...
    # yolo_training_flow can persist an uploaded dataset and then continue with
    # labels/training params in a later turn, so generic skill-level preflight
    # must not reinterpret selected data-auto-annotation as a single-image task.
