...
from app.core.coco_artifacts import build_thread_artifacts_json

router = APIRouter(prefix="/api/artifacts_json", tags=["artifacts"])
store = ArtifactStore()
...
async def list_artifacts_json(
    thread_id: str,
    request: Request,
) -> dict[str, object]:
    try:
        return build_thread_artifacts_json(
            store,
            thread_id,
...
async def get_artifact_json(thread_id: str, path: str, download: bool = False) -> Response:
    try:
