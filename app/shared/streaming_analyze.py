...
from app.shared.jetlinks_video.runtime_machine.local_vlm_runtime_machine import LocalVlmRuntimeMachine
from app.shared.jetlinks_video.runtime_machine.queue_runtime_machine import QueueRuntimeMachine
...
class StreamingAnalyze:
    ...
    def __init__(
        self,
        mode: MODEL,
        task_id: Optional[str] = None, # 上游任务ID, 基于task_id在切片目录out、原始证据帧目录evidence_images下, 再加一层目录层级便于管理
        ...
        *,
        ...
        tid = str(task_id) if task_id else "default"

        out_dir = os.path.join(storage_path, "out", tid)
        evidence_images_dir = os.path.join(storage_path, "evidence_images", tid)

        os.makedirs(out_dir, exist_ok=True)
        os.makedirs(evidence_images_dir, exist_ok=True)

        self.cut_config.out_dir = out_dir
        self.vlm_config.vlm_static_evidence_images_dir = evidence_images_dir

        # URL 前缀配置（用于前端访问；不要用 os.path.join 以免 Windows 反斜杠）
        self.vlm_config.vlm_static_evidence_images_url_prefix = f"/storage/evidence_images/{tid}"
        # 运行时机器占位
        ...
        if self.mode == MODEL.OFFLINE and self._source_kind == SOURCE_KIND.AUDIO_FILE:
            ...
            logger.info(
                "初始化离线视频分析： mode=%s, url=%s, enable_b=%s, has_audio=%s, enable_c=%s, "
                "asr_backend=%s, queue_runtime_machine=%s",
                self.mode.value, self.url, self.enable_b, self._have_audio_track,
            ...
            logger.info(
                "初始化单流常驻安防: mode=%s, url=%s, enable_b=%s, has_audio=%s, enable_c=%s, asr_backend=%s, "
                "queue_runtime_machine=%s, local_vlm_runtime_machine=%s",
                self.mode.value, self.rtsp_batch_config.polling_list[0].rtsp_url, self.enable_b,
            ...
            logger.info(
                "初始化多流轮询安防: mode=%s, polling_size=%d, enable_b=%s, polling_batch_interval=%d, "
                "local_vlm_runtime_machine=%s",
                self.mode.value, len(self.rtsp_batch_config.polling_list), self.enable_b,
    ...
    def _emit_vlm_done(self, payload: Dict[str, Any]):
        ...
        logger.info(
            "[✨✨✨VLM完整文本 seg#%s kind=%s ms=%s streaming=%s]%s",
            payload.get("segment_index"),
            payload.get("media_kind"),
            payload.get("latency_ms"),
            streaming_flag,
...
def _check_url_legal(url: str) -> None:
    ...
    if os.path.exists(url):
        return
    raise ValueError(f"不支持的媒体源地址格式: {url}，仅支持本地文件路径或 RTSP/RTMP 流")


def _determine_source_kind(url: str) -> SOURCE_KIND:
    if url.startswith(("rtsp://", "rtsps://", "rtmp://", "rtmps://")):
